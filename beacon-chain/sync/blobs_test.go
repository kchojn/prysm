...
func (c *blobsTestCase) setup(t *testing.T) (*Service, []blocks.ROBlob, func()) {
	...
	byRootRate := params.BeaconConfig().MaxRequestBlobSidecars * uint64(params.BeaconConfig().MaxBlobsPerBlock)
	byRangeRate := params.BeaconConfig().MaxRequestBlobSidecars * uint64(params.BeaconConfig().MaxBlobsPerBlock)
...
func TestTestcaseSetup_BlocksAndBlobs(t *testing.T) {
	...
	defer cleanup()
	maxed := nblocks * params.BeaconConfig().MaxBlobsPerBlock
