...
func TestSlotByRoot_Summary(t *testing.T) {
	noneSet := make([]bool, params.BeaconConfig().MaxBlobsPerBlock)
	allSet := make([]bool, params.BeaconConfig().MaxBlobsPerBlock)
	firstSet := make([]bool, params.BeaconConfig().MaxBlobsPerBlock)
	lastSet := make([]bool, params.BeaconConfig().MaxBlobsPerBlock)
	oneSet := make([]bool, params.BeaconConfig().MaxBlobsPerBlock)
	firstSet[0] = true
...
func TestAllAvailable(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			count: params.BeaconConfig().MaxBlobsPerBlock + 1,
			aa:    false,
		},
		{
			name:   "max present",
			count:  params.BeaconConfig().MaxBlobsPerBlock,
			idxSet: idxUpTo(params.BeaconConfig().MaxBlobsPerBlock),
			aa:     true,
		},
		{
			name:   "one present",
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			mask := make([]bool, params.BeaconConfig().MaxBlobsPerBlock)
